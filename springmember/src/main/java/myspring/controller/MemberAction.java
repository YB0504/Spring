...
public class MemberAction {
	...
	public String member_join_ok(@RequestParam("join_profile1") MultipartFile mf, 
		...
		// getSize()는 값을 long혈으로 전달하기 때문에 형변환을 해야한다.
...
//		System.out.println("file1="+file[1]);
	...
	if(filename != ""){	 // 첨부파일이 전송된 경우	
		...
		// extension변수에 저장된 확장자로 아래의 조건식을 작성하여도 되기때문에
		// 꼭 token을 기준으로 자르고 비교할 필요는 없다.
		...
		// 위에서 생성한 새로운 파일명을 setter메소드로 첨부파일 컬럼에 저장
		member.setJoin_profile(newfilename);

		memberService.insertMember(member);

		// redirect: = 새로운 요청을 다시 할때 사용한다.
