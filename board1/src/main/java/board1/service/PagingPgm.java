...
// 페이징에 필요한 기본변수와 파생 변수를 미리 만들어둔 클래스
// Service클래스, Controller클래스 모두에 들어가도 된다.
public class PagingPgm {
	private int total;				// 데이터 갯수 (초기값 설정되어 있는 전달 받은 기본변수)
	private int rowPerPage;			// 화면에 출력할 데이터 갯수 (초기값 설정되어 있는 전달 받은 기본변수)
	private int pagePerBlk = 10;    // 블럭당 페이지 갯수 (1개의 블럭당 10개의 페이지)
	private int currentPage;		// 현재 페이지 번호 (초기값 설정되어 있는 전달 받은 기본변수)
