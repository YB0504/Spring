...
public class BoardController {
	...
	public String list(String pageNum, Board board, Model model) {
		...
		// 총 데이터 개수 및 검색어를 포함한 데이터 개수를 구하기 위해 DTO객체 전달
		int total = bs.getTotal(board); // 검색 (데이터 갯수)
		
		int startRow = (currentPage - 1) * rowPerPage + 1;
		int endRow = startRow + rowPerPage - 1;
		
		// 객체가 생성될때 기본 변수를 매개변수로 전달하여 파생변수를 만든다.
		...
		model.addAttribute("pp", pp);	// 구해진 파생변수와 전달한 기본변수를 한번에 model로 저장하여 전달한다.
		// 검색
	...
	public String insert(Board board, Model model, HttpServletRequest request) {
		...
		if (num != 0) {		// 답변글
			...
			// 원문과 댓글에 공통적으로 적용되는 내용
			board.setNum(number);	// getMaxNum이 mapper파일에서 구한 값을 원문의 num값에 insert
